...
- **Verify ATI:** Open a browser and go to `http://192.168.1.1`. The ATI sensor is used to collect force data; the sync node will not run without a live force data stream.
...
```bash
ros2 run ves_ros_interface ves_ros_interface \
  --no-visualizer \
...
⚠️ WARNING: Always "zero" the position (return to a safe home) after testing.  
