...
class PalpationGrid(Node):
    ...
    def __init__(self):
        ...
        self.hover_pause_time = 1.0  
        self.hover_offset = 0.003    # 2mm safety margin above the peak point
        ...
        # Grid points (Replace with your full list)
        self.grid_points = [
            [-0.00900, -0.006, 0.015, 0, 0, 0, 1],
            [-0.00825, -0.006, 0.015, 0, 0, 0, 1],
            [-0.00825, -0.015, 0.015, 0, 0, 0, 1],
            # ... add all other points here ...
        ]

        # ----------------------------------------------------------------------
        # GLOBAL SAFE HEIGHT CALCULATION
        # Scans all grid points to find the highest Y (closest to home/zero)
        # ----------------------------------------------------------------------
        all_y_coords = [p[1] for p in self.grid_points]
        # Adding offset to the maximum Y found in the grid to ensure clearance
        self.safe_y = max(all_y_coords) + self.hover_offset 
        self.get_logger().info(f"🛡️ Global Safe Height (Y) set to: {self.safe_y:.5f}")
        # ----------------------------------------------------------------------
        ...
        # CSV Logging
        self.logfile = open('safe_continuous_palpation.csv', 'w', newline='')
    ...
    def ndi_callback(self, msg):
        ...
        if not self.reference_collected:
            ...
            if len(self.reference_samples) >= 10:
                ...
                self.get_logger().info("✅ Reference collected. Starting baseline...")
    ...
    def start_pause(self, label, duration, next_state, record):
        ...
        self.current_label, self.recording = label, record
    ...
    def execute_state(self):
        ...
        # 2. Move to Global Safe Height above Target
        if self.state == "MOVE_TO_HOVER":
            target = self.grid_points[self.point_index]
            hover_pose = [target[0], self.safe_y, target[2], 0, 0, 0, 1]
            self.get_logger().info(f"🚀 Moving to Safe Hover Point {self.point_index}")
        ...
        if self.state == "PAUSE_PUSH":
            self.get_logger().info(f"⬇️ Pushing Point {self.point_index}")
            self.start_pause("push", self.pause_time, "RETRACT_TO_HOVER", record=True)
            return

        # 5. Retract to Global Safe Height (Collision Clearance)
        if self.state == "RETRACT_TO_HOVER":
            target = self.grid_points[self.point_index]
            hover_pose = [target[0], self.safe_y, target[2], 0, 0, 0, 1]
            self.start_motion(self.last_command, hover_pose, "ADVANCE_POINT")
            return

        # 6. Increment Index or Finish
        if self.state == "ADVANCE_POINT":
            self.point_index += 1
            if self.point_index >= len(self.grid_points):
                self.state = "RETURN_HOME_FINAL"
            else:
                self.state = "MOVE_TO_HOVER"
            self.execute_state()
            return

        if self.state == "RETURN_HOME_FINAL":
            self.get_logger().info("🏁 Grid finished. Returning Home.")
            self.start_motion(self.last_command, self.home_pose, "DONE")
            return

        if self.state == "DONE":
            self.get_logger().info("✅ Process complete.")
            return
